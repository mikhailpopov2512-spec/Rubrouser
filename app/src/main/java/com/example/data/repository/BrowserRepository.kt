...
class BrowserRepository(private val browserDao: BrowserDao) {
    ...
    suspend fun addBlockedUrl(pattern: String, reason: String) {
        browserDao.insertBlockedUrl(BlockedUrl(pattern = pattern, reason = reason))
    }

    suspend fun deleteBlockedUrl(id: Int) {
    ...
    suspend fun getBlockedUrlsCount(): Int {
        return browserDao.getBlockedUrlsCount()
    }

    // Checking if a url is prohibited by RKN. Returns the BlockedUrl model if blocked, otherwise null.
    suspend fun checkBlockedUrl(urlString: String): BlockedUrl? {
        val patterns = browserDao.getBlockedUrlsListSync()
        val host = try {
    ...
    suspend fun restoreDefaultBlocklist() {
        browserDao.clearBlockedUrls()
        val defaults = listOf(
